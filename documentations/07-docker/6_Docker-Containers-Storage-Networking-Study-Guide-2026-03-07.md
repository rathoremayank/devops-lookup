...
Docker resource limits are enforced by Linux cgroups (control groups), a kernel feature providing resource accounting, prioritization, and limiting.

```
┌─────────────────────────────────────────────────────────────┐
│ Docker Daemon (resource policy)                              │
│ docker run --memory=512m --cpus=1.5 myapp                  │
├─────────────────────────────────────────────────────────────┤
│ cgroup v1 (memory, cpu, cpuset, pids, blkio)               │
│ /sys/fs/cgroup/memory/docker/<container-id>/               │
│ ├─ memory.limit_in_bytes: 536870912 (512MB)                │
│ ├─ memory.memsw.limit_in_bytes: 1073741824 (1GB)           │
│ ├─ cpu.cfs_quota_us: 150000 (1.5 CPUs)                     │
│ ├─ cpu.cfs_period_us: 100000 (100ms scheduling period)     │
│ └─ pids.max: 4096 (max processes)                          │
├─────────────────────────────────────────────────────────────┤
│ Kernel (enforces limits)                                     │
│ ├─ Memory manager: OOMKiller on exceed                      │
│ ├─ CPU scheduler: Prioritizes by cpu.shares                 │
│ ├─ I/O controller: Rate limits block device                 │
│ └─ Process table: Enforces pids.max                         │
└─────────────────────────────────────────────────────────────┘
```
...
Application allocates memory:
    ↓
Kernel page allocator checks cgroup limit
    ├─ Within limit: Memory allocated
    ├─ Exceeds limit: Kernel triggers memory reclamation
    └─ Still exceeds: OOMKiller terminates container (SIGKILL exit code 137)

CPU scheduling:
    ↓
CPU scheduler grants time slices (100ms periods)
    ├─ Quota: cpu.cfs_quota_us (e.g., 150000 for 1.5 CPUs)
    ├─ Process runs for allocated time
    └─ Throttled until next period (queued, not killed)

I/O limiting:
    ↓
Block device operations throttled by cgroup
    ├─ Enforced at device-level (affects all disk I/O)
    ├─ Not per-filesystem (global across paths)
    └─ Important for storage driver performance
```

### Architecture Role

Resource limits (cgroups) serve critical functions:
1. **Isolation**: Prevent one container starving others (noisy neighbor prevention)
2. **Density**: Enable high container-per-host ratios by limiting per-container
3. **Fairness**: CPU shares ensure fair distribution of oversubscribed CPU
4. **Stability**: Memory limits prevent system OOM, ensuring host resilience

### Production Usage Patterns

**Pattern 1: Conservative Limits with Headroom**

```bash
# Host has 16 CPUs, 64GB RAM
# Max capacity allocation: 80% (leave 20% headroom for kernel, system processes)
...
Explanation:
- `--cpus="3"`: Hard limit, container throttled if exceeds 3 CPU cores
- `--memory="7500m"`: Hard memory limit (OOMKill on exceed)
- `--memory-swap="8500m"`: Total memory + swap (container can use 1GB swap)
- `--memory-reservation="5000m"`: Soft limit, kernel reclaims if system under pressure

**Pattern 2: CPU Shares for Fair Allocation**

```bash
# Two services competing for CPU on same host
# Allocate according to importance ratio

# Critical service (2x priority)
docker run \
  --cpu-shares=2048 \
  api-prod:latest

# Non-critical service (1x priority)
docker run \
  --cpu-shares=1024 \
  analytics:latest

# Under contention:
# api-prod gets ~67% of available CPU
# analytics gets ~33% of available CPU
```

**Pattern 3: Device I/O Limiting**

```bash
# Prevent noisy neighbor storage I/O
docker run \
  --device-read-bps=/dev/sda:100mb \
  --device-write-bps=/dev/sda:100mb \
  --device-read-iops=/dev/sda:10000 \
  --device-write-iops=/dev/sda:10000 \
  heavy-workload:latest
```

**Pattern 4: Process Limiting**
...
docker run \
  --pids-limit=256 \
  -e max_processes=64 \
  untrusted-code:latest
```

### DevOps Best Practices

1. **Right-size limits based on profiling**:
   ```bash
   # Measure actual peak usage
   docker stats --no-stream myapp
   
   # Set limit = peak usage + 20% headroom
   # Not: arbitrary large values
   ```

2. **Monitor cgroup pressure metrics**:
   ```bash
   # Check memory pressure
   cat /sys/fs/cgroup/memory/docker/<id>/memory.stat | grep pgpgin
   
   # High pgpgin = memory churn (thrashing)
   # Indicates limit too low or memory leak
   ```

3. **Use memory reservation with hard limits**:
   ```bash
   docker run \
     --memory=512m \
   ...
   # Hard limit: 512MB, won't exceed
   # Reservation: 256MB, kernel tries to keep available
   # Gap (256-512MB): Used only if system under pressure
   ```

4. **Always test resource constraints**:
   ```bash
   # Stress test under limits
   docker run \
     --memory=256m \
     --cpus=0.5 \
     myapp \
     stress-ng --vm 1 --vm-bytes 300m --timeout 60s
   
   # Verify behavior at limits (not OOMKill surprise)
   ```

### Common Pitfalls

**Pitfall 1: No memory limits in production**

```bash
# ANTI-PATTERN: Unlimited memory
docker run myapp  # Uses all available RAM

# One container's memory leak → Host OOMs → All containers fail
# System unrecoverable, manual intervention needed
```

**Solution**:
```bash
# ALWAYS set memory limits
docker run --memory=512m --memory-swap=1024m myapp

# Predictable failure: Container killed, others continue
```

**Pitfall 2: Setting limit too close to typical usage**

```bash
# ANTI-PATTERN: Peak usage = limit
# App typically uses 800MB, set limit 800MB
docker run --memory=800m myapp

# Traffic spike → needs 850MB → OOMKill → restart → spike again → thrashing
```

**Solution**:
```bash
# Set limit = peak + headroom
docker run --memory=1000m myapp  # 800 + 200 headroom
```

**Pitfall 3: Swap disabled (memory-swap=memory)**

```bash
# ANTI-PATTERN: Swap = memory limit value
docker run --memory=512m --memory-swap=512m myapp

# No swap available
# Memory pressure → OOMKill immediately, no chance to spill to swap
```

**Solution**:
```bash
# Allow swap as safety valve
docker run --memory=512m --memory-swap=1024m myapp

# Memory pressure:
#   ├─ 0-512MB: Physical RAM
#   ├─ 512-1024MB: Swap (slower but doesn't kill)
#   └─ >1024MB: OOMKill
```

**Pitfall 4: Ignoring CPU scheduling overhead**

```bash
# ANTI-PATTERN: Allocating 100% CPU
# 8 cores × 8 containers = --cpus=1 each = 100% allocated
# But scheduling overhead, context switches consume 5-10%
# Result: All containers throttled, poor performance
```

**Solution**:
```bash
# Allocate ~80% of total
# 8 cores × 0.8 = 6.4 CPUs available
# Can run ~6 containers with --cpus=1
```
...
Docker security relies on multiple kernel mechanisms stacked together:

```
┌──────────────────────────────────────────────────────────┐
│ Container Application (untrusted code)                    │
├──────────────────────────────────────────────────────────┤
│ Linux Namespaces (isolation layer)                       │
│ ├─ PID namespace: Process(id) isolation                  │
│ ├─ Network namespace: Network stack isolation            │
│ ├─ Mount namespace: Filesystem isolation                 │
│ ├─ UTS namespace: Hostname isolation                     │
│ ├─ IPC namespace: Inter-process communication isolation  │
│ ├─ User namespace: UID/GID remapping (optional)          │
│ └─ Cgroup namespace: cgroup hierarchy hiding             │
├──────────────────────────────────────────────────────────┤
│ Kernel Capabilities (fine-grained privileges)            │
│ ├─ Drop: CAP_NET_ADMIN, CAP_SYS_MODULE, etc.            │
│ ├─ Add: Only needed capabilities                         │
│ └─ Result: Least-privilege container execution           │
├──────────────────────────────────────────────────────────┤
│ Mandatory Access Control (MAC)                           │
│ ├─ SELinux: Targeted policies per container              │
│ ├─ AppArmor: Profile-based restrictions                  │
│ └─ Impact: Restrict syscalls, file access beyond UID    │
├──────────────────────────────────────────────────────────┤
│ Read-only Rootfs (optional hardening)                    │
│ ├─ Container rootfs mounted read-only                    │
│ ├─ Only /tmp, /var/tmp writable                          │
│ └─ Prevents code injection attacks                       │
├──────────────────────────────────────────────────────────┤
│ Linux Kernel (enforces all restrictions)                 │
...
Security mechanisms serve:
1. **Blast Radius Containment**: Compromised container can't break out to host
2. **Multi-Tenant Isolation**: One user's container can't access another's
3. **Privilege Escalation Prevention**: Container can't gain host-level access
4. **Compliance**: Audit trails, access controls for regulated workloads

### Production Usage Patterns

**Pattern 1: Hardened Container with Minimal Capabilities**

```bash
# Production configuration: Tight security posture
docker run \
  --cap-drop=ALL \
...
- `--cap-add`: Add only necessary (NET_BIND_SERVICE for port binding, CHOWN for files)
- `--read-only`: Filesystembread-only, prevents modifications
- `--tmpfs`: Writable areas with security flags
- `--user=1000:1000`: Non-root execution
- `--security-opt no-new-privileges:true`: Child processes can't escalate

**Pattern 2: User Namespace Remapping (Multi-Tenant)**
...
# Inside container, UID 0 (root) = UID 100000 (unprivileged user on host)
docker run --rm alpine id
# Output: uid=0(root) gid=0(root) groups=0(root)
# But on host: ps aux | grep container shows UID 100000:100000
```

Result: Even `root` inside container is just unprivileged user on host.

**Pattern 3: AppArmor Profile Enforcement**

```bash
# Create AppArmor profile
cat > /etc/apparmor.d/docker-myapp << 'AAPROFFILE'
...
**Pattern 4: Read-Only Rootfs with Explicit Writable Paths**
...
# Result:
# / is read-only (can't modify app binary, libraries, config)
# /tmp, /var/tmp ephemeral (cleared on container stop)
# /var/log/app, /data persistent (designed for writes)
```
...
2. **Drop unnecessary capabilities**:
   ...
   # Check with --cap-drop=ALL, capture errors, add back as needed
   ```

3. **Use read-only mode for immutable services**:
   ```bash
   # Web server (shouldn't write to rootfs)
   docker run \
     --read-only \
     --tmpfs /tmp \
     nginx:latest
...
4. **Implement container image scanning**:
   ...
   # Blocks deployment if HIGH+ vulnerabilities found
   ```

5. **Use user namespace remapping for multi-tenant**:
   ```bash
   # In daemon.json
   {
     "userns-remap": "default"
   ...
   UID 0 in container = unprivileged UID on host
   ```
...
```bash
# ANTI-PATTERN: Default user is root
FROM ubuntu:20.04
ENTRYPOINT ["/app/start.sh"]
# No USER directive = runs as UID 0
```

**Risk**: Container escape → Attacker becomes host root
...
**Pitfall 2: Using `--privileged` by default**

```bash
# ANTI-PATTERN: Blanket privilege grant
...
**Solution**: Only for specific use cases (Docker-in-Docker, Kubernetes nodes)

**Pitfall 3: Ignoring SELinux/AppArmor messages**

```bash
# Container fails with permission denied
# "Ignore, use :Z flag"

docker run -v /share:/data:Z myapp
```

**Risk**: `:Z` flag weakens isolation; changes SELinux context globally

**Solution**: Understand SELinux labels, don't blindly disable

**Pitfall 4: Storing secrets in environment variables**

```bash
# ANTI-PATTERN: Secret in ENV
FROM ubuntu
ENV DB_PASSWORD=supersecret
```

**Risk**: Secret in image layer history, visible in `docker history`, inspectable
...
# Only readable by container, not in environment
```

**Pitfall 5: Container image with unpatched vulnerabilities**

```bash
# ANTI-PATTERN: Base image never updated
FROM ubuntu:16.04  # EOL, unpatched CVEs
RUN apt-get install myapp
```

**Risk**: Known exploits in base image

**Solution**:
```dockerfile
# Regular updates
FROM ubuntu:22.04  # LTS, regular patches
...
# Run with all caps, remove one by one
docker run myapp 2>&1 | grep "operation not permitted"

# Add back capability
docker run --cap-add=CAP_NAME myapp
```
...
# On host for running container
docker run -it myapp capsh --print
```
...
# Re-label with :z (shared) or :Z (private)
docker run -v /share:/data:z myapp  # shared context
```

---

**Document Status**: All sections complete. Study guide now includes:
✅ Foundational Concepts
✅ Container Commands & Lifecycle (detailed)
✅ Docker Storage (all variants, data management)
✅ Docker Networking (all driver types)
✅ Container Resource Management & Cgroups (production patterns, pitfalls)
✅ Container Security & Isolation (namespaces, capabilities, MAC)
✅ Hands-on Scenarios
✅ Senior-level Interview Questions (9 comprehensive)

**Last Updated**: 2026-03-07  
**Intended for**: DevOps Engineers with 5–10+ years experience  
**Target Platform**: Docker 20.10+ on Linux (kernel 4.15+)  
**Approx Reading Time**: 5-7 hours (production-ready comprehensive depth)  
**Total Content**: ~5,500 lines, 20+ diagrams, 60+ code examples, advanced troubleshooting
